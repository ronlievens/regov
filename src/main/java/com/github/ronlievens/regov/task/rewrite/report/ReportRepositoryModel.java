...
public class ReportRepositoryModel implements Comparable<ReportRepositoryModel> {
    ...
    public static ReportRepositoryModel create(final AzureRepository azureRepository) {
        if (azureRepository == null) {
            return null;
        }

        val report = new ReportRepositoryModel();
        report.setId(azureRepository.getId());
        report.setName(azureRepository.getName());
        report.setUrl(azureRepository.getUrl());
        report.setProjectId(azureRepository.getProject().getId());
        report.setProjectName(azureRepository.getProject().getName());
        report.setOrganizationName(azureRepository.getProject().getOrganizationName());
        report.setChange(new ReportRepositoryChangeModel());
        report.setAcceptance(new ReportRepositoryChangeModel());
        report.setProduction(new ReportRepositoryChangeModel());
        return report;
    }
